...
class ClockPID_c {
  public:
    ClockPID_c() : count(0), timestamp_wraps(0), last_timestamp(0) { }
    float add_sample(uint32_t timestamp, uint64_t realSecond, int64_t corrected_offset);
  ...
  private:
    uint64_t timestamps[NTPPID_MAX_COUNT]; // in counts
    uint64_t realSeconds[NTPPID_MAX_COUNT]; // ntp 64bit timestamp
    int64_t rawOffsets[NTPPID_MAX_COUNT]; // in counts
    int64_t corrected_offsets[NTPPID_MAX_COUNT]; // in fractional ntp seconds

    uint32_t timestamp_wraps, last_timestamp;
    uint32_t count;
    ...
    float chisq(struct linear_result lin, uint64_t *timestampDurations);
    struct deriv_calc calculate_d();
    float calculate_i();
    float limit_500(float factor);
    void make_room();
    float average(int64_t *points);
    float average(uint64_t *points);
};

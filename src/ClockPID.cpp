...
float ClockPID_c::average(int64_t *points) {
  double sum = 0;
...
float ClockPID_c::average(uint64_t *points) {
  double sum = 0;
...
struct linear_result ClockPID_c::theil_sen(float avg_ts, float avg_out) {
  ...
  for(uint32_t i = 0; i < (count - 1); i++) {
    for(uint32_t j = i+1; j < count; j++) {
      uint64_t localDuration = timestamps[j] - timestamps[i];
...
float ClockPID_c::chisq(struct linear_result lin, uint64_t *timestampDurations) {
  float chisq_r = 0;
...
struct deriv_calc ClockPID_c::calculate_d() {
  ...
  struct deriv_calc d;
  uint64_t timestampDurations[NTPPID_MAX_COUNT];

  rawOffsets[0] = 0;
  timestampDurations[0] = 0;
  for(uint32_t i = 1; i < count; i++) {
    uint64_t remoteDuration = realSeconds[i] - realSeconds[0];
    timestampDurations[i] = timestamps[i] - timestamps[0];
    // translate from 64bit ntp timestamps to 32 bit counter, trying to avoid an overflow
    remoteDuration = (remoteDuration * (COUNTSPERSECOND >> 10)) >> 22;
    rawOffsets[i] = remoteDuration - timestampDurations[i];
  }
...
float ClockPID_c::add_sample(uint32_t timestamp, uint64_t realSecond, int64_t corrected_offset) {
  ...
  unwrapped_timestamp = ((uint64_t)timestamp_wraps << 32) | timestamp;
  ...
  timestamps[count] = unwrapped_timestamp;
